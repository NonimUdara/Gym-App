...
export default class Home extends Component {
  ...
  filterData(posts, searchKey) {
    ...
    const result = posts.filter((post) =>
      ...
      post.request.toLowerCase().includes(searchKey) ||
      post.name.toUpperCase().includes(searchKey) ||
      post.email.toUpperCase().includes(searchKey) ||
      post.request.toUpperCase().includes(searchKey)
    )
            ...
            <div className="collapse navbar-collapse" id="navbarNav">
              <ul className="navbar-nav">
                <li className="nav-item">
                  ...
                  <a className="navbar-brand" aria-current="page" style={{ fontWeight: 'small', fontSize: 20 }} href="/trainerpanel">Trainer Panel</a>
                </li>
                <li className="nav-item">
